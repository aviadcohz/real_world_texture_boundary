"""
Training loop for Qwen2SAM v2_tracker (Stage 2: DETR + SAM3 Tracker).
...
Usage:
  ...
  python -m qwen2sam.training.train_v2_tracker --config qwen2sam/configs/v2_tracker.yaml
"""
...
# -------------------------------------------------------------------- #
#  Tracker Collator (extends V2Collator with POINT tokens)               #
# -------------------------------------------------------------------- #

class V2TrackerCollator:
    """
    Collate function for v2_tracker training.

    Extends V2Collator pattern with:
      - POINT tokens appended to assistant text
      - point_positions (B, 2*N) in batch output
    """
    ...
    def _make_assistant_text(self, sample: dict) -> str:
        if self.inference:
            base = "The transition is from texture <SEG_A> to texture <SEG_B>."
        else:
    ...
    def __call__(self, samples: list[dict]) -> dict:
        ...
        # ---- Find SEG token positions (last occurrence) ----------------- #
        seg_a_positions = []
        seg_b_positions = []
...
TRACKER_METRICS = [
    "total", "detr_loss", "tracker_loss", "align_loss", "recon_loss", "lm_loss",
    "detr_mask_iou", "tracker_iou",
    "alignment_loss", "alignment_acc", "reconstruction_loss",
]
...
def save_tracker_checkpoint(model, optimizer, scheduler, scaler, epoch, metrics, path):
    """Save v2_tracker checkpoint with all components."""
    state = {
        "epoch": epoch,
        "metrics": metrics,
    ...
    # LoRA weights (including POINT token embeddings)
    lora_state = {
        k: v for k, v in model.base.qwen.state_dict().items()
        if "lora" in k.lower() or "seg_" in k.lower() or "point_" in k.lower()
        or "embed_tokens" in k.lower()
    }
    state["qwen_lora_state_dict"] = lora_state

    # Tracker-specific components
    state["coord_head_state_dict"] = model.coord_head.state_dict()
    state["point_projector_state_dict"] = model.point_projector.state_dict()
...
def load_tracker_checkpoint(model, optimizer, scheduler, scaler, path):
    """Load v2_tracker checkpoint."""
    ...
    if "point_projector_state_dict" in ckpt:
        model.point_projector.load_state_dict(ckpt["point_projector_state_dict"])
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, cfg, device, epoch):
    """Run one training epoch for v2_tracker."""
    # Train mode for components that need it
    model.base.qwen.train()
    model.base.projector.train()
    model.coord_head.train()
    model.point_projector.train()
    # SAM components in eval mode (avoids activation checkpointing conflicts)
    ...
    for step, batch in enumerate(loader):
        ...
        # Skip samples with missing token positions
        ...
        meters["recon_loss"].update(loss_metrics.get("recon_loss", 0), B)
        ...
        if "reconstruction_loss" in loss_metrics:
            meters["reconstruction_loss"].update(loss_metrics["reconstruction_loss"], B)
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2SAM v2_tracker Training (Stage 2)")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--data_root", type=str, default=None)
    parser.add_argument("--resume", type=str, default=None)
    parser.add_argument("--resume_weights_only", type=str, default=None,
                        help="Load model weights from checkpoint but use fresh optimizer/scheduler")
    ...
    # ---- Build model --------------------------------------------------- #
    print("Building Qwen2SAM v2_tracker model...")
    model = Qwen2SAMv2Tracker(cfg, device=str(device))
    ...
    if loss_cfg.get("alignment_weight", 0.0) > 0:
        ...
        embedder_type = cfg.get("tracker", {}).get("align_embedder", "qwen")
    ...
    start_epoch = 0
    if args.resume:
        start_epoch = load_tracker_checkpoint(
            model, optimizer, scheduler, scaler, args.resume
        ) + 1
    elif args.resume_weights_only:
        # Load model weights only — fresh optimizer/scheduler for a new training phase
        ...
        if "sam_mask_decoder_state_dict" in ckpt:
            model.sam_mask_decoder.load_state_dict(ckpt["sam_mask_decoder_state_dict"])
        print(f"  Loaded weights from {args.resume_weights_only} (fresh optimizer/scheduler)")

    # ---- Checkpoint dir ------------------------------------------------ #
    ckpt_cfg = cfg.get("checkpoint", {})
    ckpt_dir = Path(ckpt_cfg.get("dir", "checkpoints/v2_tracker"))
    ...
    print(f"\n{'='*60}")
    print("Qwen2SAM v2_tracker — Stage 2 Training Start")
    print(f"  Architecture: DETR + Tracker (Qwen point coords)")
    print(f"  DETR weight: {loss_cfg.get('detr_weight', 0.5)}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        plot_training_curves(training_history, str(ckpt_dir), phase_name="V2 Tracker")
        ...
        # Track best by tracker IoU (the metric we care about)
